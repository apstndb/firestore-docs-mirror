...
Datastore is built on top of Google's NoSQL database, [Bigtable](https://docs.cloud.google.com/bigtable) , and is subject to Bigtable's performance characteristics. Bigtable scales by sharding rows onto separate tablets, and these rows are lexicographically ordered by key.
