---
name: documents/docs.cloud.google.com/datastore/docs/use-cmek
uri: https://docs.cloud.google.com/datastore/docs/use-cmek
...
Before you can create a CMEK-protected Datastore mode database, you must complete the following steps:

1.  [Request access to the Datastore mode CMEK feature](https://docs.cloud.google.com/datastore/docs/use-cmek#request-cmek-access) .
2.  [Create (or retrieve) a Datastore mode service agent](https://docs.cloud.google.com/datastore/docs/use-cmek#create-service-agent) .
3.  [Create a CMEK key](https://docs.cloud.google.com/datastore/docs/use-cmek#create-key) .
4.  [Configure IAM settings for that key](https://docs.cloud.google.com/datastore/docs/use-cmek#set-iam) .
...
> **Note:** The `gcloud` commands on this page show placeholders for two Google Cloud projects: the project that contains Datastore mode databases, `  FIRESTORE_PROJECT  ` , and the project that contains your CMEK key(s), `  KMS_PROJECT  ` . However, you can use the same project for your Datastore mode databases and CMEK keys.

### Request access

Before you create a Datastore mode service agent, request access to the CMEK feature by filling in [this form](https://docs.google.com/forms/d/e/1FAIpQLSfKs8wJf4IXu1NizvfyU2vT59JDbdPvkehMVZ2ab5l_aDLIIA/viewform?resourcekey=0-O15dlRFvA0JIDmh6VFUEcA) .

### Create a Datastore mode service agent

Before you create a CMEK key, you must have a Datastore mode [service agent](https://cloud.google.com/iam/docs/service-agents) , which is a type of Google-managed service account that Datastore mode uses to access the key.

Run the [services identity create](https://cloud.google.com/sdk/gcloud/reference/beta/services/identity/create) command to create the service agent that Datastore mode uses to access the CMEK key on your behalf. This command creates the service account if it does not already exist, then displays it.

    gcloud beta services identity create \
        --service=firestore.googleapis.com \
        --project FIRESTORE_PROJECT

Replace `  FIRESTORE_PROJECT  ` with the project you plan to use for your Datastore mode databases.
...
The Cloud KMS [key location](https://cloud.google.com/kms/docs/locations) must be the same as the location of the Datastore mode database that it will be used with.

  - For [regional database locations](https://docs.cloud.google.com/datastore/docs/locations#location-r) , use the same location name for key ring, key, and database because the location names have a one-to-one mapping.
    
    For example, if you want to create a CMEK-protected database in `us-west1` , create a key ring and key in `us-west1` .

  - For [multi-region database locations](https://docs.cloud.google.com/datastore/docs/locations#location-r) , use the location name of the [KMS multi-region location](https://cloud.google.com/kms/docs/locations) :
    
      - Use the Cloud KMS `us` multi-region location for the Datastore mode `nam5` multi-region location.
      - Use the Cloud KMS `europe` multi-region location for the Datastore mode `eur3` multi-region location.
...
3.  Enter the email-formatted ID for your Datastore mode service agent.
...
After your CMEK keys are created and configured, you can create a CMEK-protected database. Existing Datastore mode databases that are protected by Google default encryption can't be converted to use CMEK.
...
9.  If you are prompted to grant key permission to the Datastore mode service account, click **Grant** . To create a CMEK database, your Datastore mode service account must be granted the `cloudkms.cryptoKeyEncrypterDecrypter` role.
...
Replace the following:
  ...
  - `  FIRESTORE_DATABASE_LOCATION  ` with the Datastore mode location for the database
  ...
  - `  FIRESTORE_PROJECT  ` with the project to use for your Datastore mode database

### REST API

HTTP request:

POST https://firestore.googleapis.com/v1/projects/{FIRESTORE\_PROJECT}/databases
...
For more information about other fields, see the [`database create` page](https://docs.cloud.google.com/datastore/docs/reference/rest/v1/projects.databases/create) .
...
To create a CMEK-enabled database, use the **KMS Key Name** field. If you don't specify the `--kms-key-name` parameter, Datastore mode creates a non-CMEK database by default.
...
Confirm that your Datastore mode database is protected with Firebase CLI:

    firebase firestore:databases:get DATABASE_ID --project FIRESTORE_PROJECT

The following CMEK information appears in the response message:

  - The **KMS Key Name** field provides the full key resource name that is used to encrypt your Datastore mode CMEK database.
...
Replace the following:
  ...
  - `  DATABASE_ID  ` with the ID of your database
  - `  FIRESTORE_PROJECT  ` with the project to use for your Datastore mode database
  - `  FIRESTORE_LOCATION  ` with the location of your Datastore mode database
  - `  BACKUP_ID  ` with the ID of your backup
  - `  KMS_PROJECT  ` with the project that contains your CMEK key
  - `  KMS_LOCATION  ` with the location that contains your CMEK key and key ring
  - `  KMS_KEYRING_ID  ` with the ID of your CMEK key ring

Confirm that your restored Datastore mode database is CMEK-encrypted:
...
1.  [View the key versions in use for a database](https://docs.cloud.google.com/datastore/docs/use-cmek#view-key) .
...
[Cloud KMS Data Access audit logs](https://cloud.google.com/kms/docs/audit-logging) show you when Datastore mode or any other products that are configured to use your CMEK key make encrypt/decrypt calls to Cloud KMS. Datastore mode does not issue an encrypt/decrypt call on every data request, but instead maintains a poller that checks the key periodically. The polling results appear in the audit logs.
...
To specify encryption compliance requirements for Datastore mode databases in your organization, use a [CMEK organization policy constraint](https://cloud.google.com/kms/docs/cmek-org-policy) .

### Require CMEK protection

Configure `constraints/gcp.restrictNonCmekServices` to require CMEK for Datastore mode database creation. Set the constraint to `deny` and add `firestore.googleapis.com` to the deny list, for example:
...
## What's next
  ...
  - [Read the overview of Datastore mode and CMEK](https://docs.cloud.google.com/datastore/docs/cmek)
