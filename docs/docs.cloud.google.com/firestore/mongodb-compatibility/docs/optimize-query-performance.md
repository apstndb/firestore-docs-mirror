...
If an index is used for a query, but the query engine is still fetching and discarding many documents, as identified by a Scan node that returns many records followed by a [Filter node](https://docs.cloud.google.com/firestore/mongodb-compatibility/docs/query-explain-reference#filter) that returns few records, this is a sign that the query predicate satisfied using the index is not selective. To create a more suitable index, see [Create indexes](https://docs.cloud.google.com/firestore/mongodb-compatibility/docs/optimize-query-performance#create_indexes) .
...
If you are using the `pipeline` option in the `$lookup` stage with `$match` stages, create an index on the fields involved in the foreign collection to avoid a full collection scan:
...
Follow the index management documentation to [create indexes](https://docs.cloud.google.com/firestore/mongodb-compatibility/docs/indexing) . To ensure your query can use indexes, create regular (not Multikey) indexes with fields in the following order:
