...
A backup does not contain database [time to live policies.](https://docs.cloud.google.com/firestore/mongodb-compatibility/docs/ttl) A backup resides in the same location as the source database.
...
Replace the following:
  ...
  - DATABASE\_ID : The ID of the database to back up. Set to `'(default)'` for the default database.
  - BACKUP\_SCHEDULE\_ID : The ID of a backup schedule. You can view the ID of each backup schedule when you [list all backup schedules](https://docs.cloud.google.com/firestore/mongodb-compatibility/docs/backups#list_backup_schedules) .
...
Note that deleting a backup schedule won't delete backups already created by this schedule. You can wait for them to expire after their retention period, or to manually delete a backup, see [delete backup](https://docs.cloud.google.com/firestore/mongodb-compatibility/docs/backups#delete_backup) .
...
Replace `  LOCATION  ` with the name of a Firestore with MongoDB compatibility location.
...
Replace the following:
  ...
  - BACKUP\_ID : The ID of a backup. You can view the ID of each backup when you [list all backups](https://docs.cloud.google.com/firestore/mongodb-compatibility/docs/backups#list_backups) .
...
A restore operation writes the data from a backup to a new Firestore with MongoDB compatibility database.
...
After you finish restoring, you should do the following:
  ...
  - Verify that appropriate [IAM controls](https://docs.cloud.google.com/firestore/mongodb-compatibility/docs/security/iam) are applied to your new database.

  - If you previously used [TTL](https://docs.cloud.google.com/firestore/mongodb-compatibility/docs/ttl) policies, reapply them to the new database. TTL policies are not included in backups and are not automatically reapplied to restored databases.
