...
A backup does not contain database [time to live policies.](/firestore/mongodb-compatibility/docs/ttl) A backup resides in the same location as the source database.
...
After you finish restoring, you should do the following:
  ...
  - Verify that appropriate [IAM controls](/firestore/mongodb-compatibility/docs/security/iam) are applied to your new database.

  - If you previously used [TTL](/firestore/mongodb-compatibility/docs/ttl) policies, reapply them to the new database. TTL policies are not included in backups and are not automatically reapplied to restored databases.
