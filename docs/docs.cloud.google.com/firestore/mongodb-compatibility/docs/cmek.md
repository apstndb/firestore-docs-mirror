...
There are no additional Firestore with MongoDB compatibility costs for using CMEK-protected database and the [Firestore with MongoDB compatibility pricing](https://cloud.google.com/firestore/enterprise/pricing) continues to apply.
...
A backup uses the same encryption mechanism as the database from which you created it. When a CMEK-protected Firestore with MongoDB compatibility database creates a backup, it encrypts the backup with the primary key version used at the time of backup creation.
...
You can use key tracking to view resources, for example, Firestore with MongoDB compatibility databases, that a key protects. For more information about key tracking, see [View key usage](https://cloud.google.com/kms/docs/view-key-usage) .
