---
name: documents/docs.cloud.google.com/firestore/native/docs/transaction-data-contention
uri: https://docs.cloud.google.com/firestore/native/docs/transaction-data-contention
title: Data contention in transactions
...
Firestore guarantees serializable isolation of transactions. Transactions in Firestore are serialized and isolated by commit time.

### Serializable isolation by commit time

Firestore assigns each transaction a commit time which represents a single point in time. When Firestore commits a transaction's changes to the database, you can assume all reads and writes within the transaction take place exactly at the commit time.
