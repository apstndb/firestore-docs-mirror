...
The [Firestore data model](/firestore/docs/data-model) consists of collections that contain documents. A document contains a set of key-value pairs.
...
If a document in `  users  ` has [subcollections](/firestore/docs/concepts/structure-data#subcollections) , and a field in one of those subcollections' documents is changed, the `  {username}  ` wildcard is *not* triggered. If your goal is to respond to events in subcollections also, use the multi-segment wildcard `  {username=**}  ` .
...
See [examples](#examples) for how to use Firestore in Native Mode events to trigger a Cloud Run function.
