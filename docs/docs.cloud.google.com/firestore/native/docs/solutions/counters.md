...
In Firestore, you can't update a single document at an unlimited rate. If you have a counter based on single document and frequent enough increments to it you will eventually see contention on the updates to the document. See [Updates to a single document](https://docs.cloud.google.com/firestore/native/docs/best-practices#updates_to_a_single_document) .
...
**Note:** This product is not available on watchOS and App Clip targets.
    ...
    @endFIRSolutionCountersViewController.m
...
Android
    ...
    // counters/${ID}/shards/${NUM}
    data class Shard(var count: Int)SolutionCounters.kt
...
### Ruby
    ...
    puts "Distributed counter shards collection created."distributed_counters.rb
...
### Ruby
    ...
    puts "Count value is #{count}."distributed_counters.rb

## Limitations

The solution shown above is a scalable way to create shared counters in Firestore, but you should be aware of the following limitations:
