...
In Firestore, you can't update a single document at an unlimited rate. If you have a counter based on single document and frequent enough increments to it you will eventually see contention on the updates to the document. See [Updates to a single document](/firestore/native/docs/best-practices#updates_to_a_single_document) .
