---
name: documents/docs.cloud.google.com/firestore/native/docs/solutions/aggregation
uri: https://docs.cloud.google.com/firestore/native/docs/solutions/aggregation
...
The solution shown above demonstrates aggregating data using the Firestore client library, but you should be aware of the following limitations:
  ...
  - **Performance** - If your transaction contains multiple read, write, and update operations, it could require multiple requests to the Firestore backend. On a mobile device, this could take significant time.
...
If client-side transactions are not suitable for your application, you can use a [Cloud Function](https://docs.cloud.google.com/firestore/native/docs/extend-with-functions) to update the aggregate information each time a new rating is added to a restaurant:
