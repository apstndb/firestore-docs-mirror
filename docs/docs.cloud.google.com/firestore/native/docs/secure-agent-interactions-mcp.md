...
For the strongest defense, combine IAM roles with the granular access controls offered by the database itself. This ensures that even if an attacker compromises the agent's IAM token, the scope of damage is limited by the database engine's internal permissions—for example, preventing a Delete Database command.
...
Firestore provides Model Armor to enforce safety boundaries at the platform level. You must enable and configure these settings.
...
While layered controls like IAM and database-native roles are designed to prevent destructive actions, you must have a recovery plan in case those defenses fail. A compromised or naive agent with write permissions (an "Agent-Only" risk) might be tricked into executing a destructive command like Delete Database or corrupting data.
...
In addition to Cloud Audit Logs, make sure that your application code logs the following data for every agent decision:
  ...
  - **Raw command:** the exact command—for example, a Firestore query or document path—generated by the LLM.
