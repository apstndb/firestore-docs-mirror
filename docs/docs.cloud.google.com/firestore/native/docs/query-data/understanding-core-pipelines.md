---
name: documents/docs.cloud.google.com/firestore/native/docs/query-data/understanding-core-pipelines
uri: https://docs.cloud.google.com/firestore/native/docs/query-data/understanding-core-pipelines
title: Query interfaces overview
...
<tr class="even">
<td><strong>Index Sparsity</strong></td>
<td>All indexes are <strong>sparse</strong> . An index entry exists only if <strong>all</strong> fields defined in the index are present in the document.</td>
<td>The default is <strong>dense</strong> , where an index entry exists even if no fields defined in the index are present in the document.
<p>Additionally, sparse indexes can be enabled, where an index entry exists if at least one field in the index is present in the document.</p></td>
</tr>
<tr class="odd">
