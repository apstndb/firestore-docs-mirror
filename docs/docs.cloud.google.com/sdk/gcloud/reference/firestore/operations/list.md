...
`gcloud firestore operations list` \[ `  --database  ` = `  DATABASE  ` ; default="(default)"\] \[ `  --filter  ` = `  EXPRESSION  ` \] \[ `  --limit  ` = `  LIMIT  ` ; default=100\] \[ `  --page-size  ` = `  PAGE_SIZE  ` ; default=100\] \[ `  --sort-by  ` =\[ `  FIELD  ` , …\]\] \[ `  --uri  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
