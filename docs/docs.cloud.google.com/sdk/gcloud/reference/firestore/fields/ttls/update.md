...
`gcloud firestore fields ttls update` ( `  FIELD  ` : `  --collection-group  ` = `  COLLECTION_GROUP  ` `  --database  ` = `  DATABASE  ` ) ( `  --disable-ttl  ` | \[ `  --enable-ttl  ` : `  --expiration-offset  ` = `  EXPIRATION_OFFSET  ` \]) \[ `  --async  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
