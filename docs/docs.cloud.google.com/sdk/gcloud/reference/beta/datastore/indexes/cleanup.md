---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/beta/datastore/indexes/cleanup
uri: https://docs.cloud.google.com/sdk/gcloud/reference/beta/datastore/indexes/cleanup
title: gcloud beta datastore indexes cleanup
...
`gcloud beta datastore indexes cleanup` `  INDEX_FILE  ` \[ `  --database  ` = `  DATABASE  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]

DESCRIPTION

`(BETA)` This command removes unused datastore indexes based on your local index configuration. Any indexes that exist that are not in the index file will be removed.
