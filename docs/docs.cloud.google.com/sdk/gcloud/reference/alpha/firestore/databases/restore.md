...
`  gcloud alpha firestore databases restore  ` `  --destination-database  ` = `  DESTINATION_DATABASE  ` `  --source-backup  ` = `  SOURCE_BACKUP  ` \[ `  --tags  ` =\[ `  KEY  ` = `  VALUE  ` , …\]\] \[ `  --encryption-type  ` = `  ENCRYPTION_TYPE  ` : `  --kms-key-name  ` = `  KMS_KEY_NAME  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
``` text
gcloud alpha firestore databases restore --source-backup=projects/PROJECT_ID/locations/LOCATION_ID/backups/BACKUP_ID --destination-database=DATABASE_ID --tags=key1=value1,key2=value2
```
