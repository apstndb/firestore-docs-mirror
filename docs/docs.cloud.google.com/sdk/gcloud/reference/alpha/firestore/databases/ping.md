...
`  gcloud alpha firestore databases ping  ` `  --database  ` = `  DATABASE  ` \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
``` text
gcloud alpha firestore databases ping --database=testdb
```
...
REQUIRED FLAGS
  ...
  - `  --database  ` = `  DATABASE  `  
    ...
    ``` text
    gcloud alpha firestore databases ping --database='foo'
    ```
